...
def compute_trajLf(max_steps, **kwargs):
    ...
    if 'force_mag' in kwargs:
        ...
        for _ in range(max_steps):
            action=pi
            ob, r, done, _ = env.step(action)
            noise_x = np. random. normal(0,0.4,1)
            noise_v=np. random. normal(0,0.1,1)
            ob[0]=ob[0]+noise_x
            ob[1]=ob[1]+noise_v
            reward = reward+r
            trajLf.append(ob)
...
rand_nums3=[20, 22, 24, 26, 28, 30, 32, 34, 36, 38,54,87,46,27,17]
...
rand_nums6=[1461, 8194, 6927, 5075, 4903, 3799, 6268, 8155, 5502, 1187,2147,1577,3178,6084,2893]
...
rand_nums10=[317957, 510020, 545416, 613511, 673274, 619204, 630790, 627544,
       127016, 390172,123658,145698,625893,965823,145896]
rand_num=[rand_nums1,rand_nums2,rand_nums4]
