...
def compute_trajLf(**kwargs):
    ...
    while done==False:
        ...
        action=pi
        ob, reward, done, _ = env.step(action)  
        noise=np.array([0.5*np.random.rand(1),0.2*np.random.rand(1)])
        n_v=noise[1]
        n_x=noise[0]
        ob[0]+=n_x
        ob[1]+=n_v
...
rand_num=[rand_nums8,rand_nums9 ,rand_nums10]  #
# ,rand_nums1,rand_nums2 ,rand_nums3 ,rand_nums4,rand_nums5,rand_nums6 ,rand_nums7 ,
...
for m in range(3):
  for r in rand_num[m]:
      ...
      TM_ns = test_module(bounds=bounds,suth=lambda x0: sutH(x0,max_steps=350),sutl=lambda x0: sutL(x0,max_steps=350), 
                          f_tree = node,init_sample =3, with_ns=True, exp_weight=2, normalizer=True)
      TM_ns.initialize()
      TM_ns.run_BO(11)
      nums.append(n)
